...
### 免责声明（只保留标准一行，**不写「本期导读」**）

免责声明区**只保留模板里那一行标准声明**，写完直接进 `## 模型 & 产品`，中间**不再加第二个 `>` 引用块**。

> **废弃说明**：07-22 到 08-08 期曾在免责声明后多写一段「本期导读」（重申栏目范围 + 时间窗、本窗取舍、选题观察）。**该段已废弃、不再出现**——它把「栏目范围」「本期收了啥」这些读者往下看就知道的信息又复述一遍，是冗余的套话高发区。范围与免责已由那一行标准声明覆盖，取舍与时间窗放进各篇 meta 行和「论文精选」引子即可。

下面三条**诚实底线**不随导读段废弃而消失——它们适用于任何串联性表述（论文精选引子、各篇之间的呼应、front matter 的 `description`）：

- **只陈述、不升华**：可以点出「这批论文大致落在 X 类」，但别把 6~12 篇松散工作缝成「某某正在往 X 走」这类大结论——那是硬凑的主题句，经不起推敲。若要写一句取样观察，后缀「这只是本期取样，不代表当周主线」之类的限定。
...
- **不硬凑主题、不夸大**：各篇的串联、论文精选引子、front matter 的 `description`，都别把松散工作拔高成「行业正在往 X 走」的大论点；覆盖缺口如实写、来源份量别抬高（详见 ⑤「免责声明」的三条诚实底线）。**不写「本期导读」引用块**（已废弃，见 ⑤）。总结段套话最多，写完对着它自问一句「这话经得起推敲吗」。
